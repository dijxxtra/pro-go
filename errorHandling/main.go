...
func progressCategory(categories []string, outChan chan<- CategoryCountMessage) {
	defer func() {
...
func main() {
	categories := []string{"Watersports", "Chess", "Phones"}
	channel := make(chan CategoryCountMessage)
	go progressCategory(categories, channel)
